...
def trainingModel(df_final, variables_costos, dummy_variables, ratio_variables, model_type, target):
    ...
    if model_type == 'xgboost':
        for i in range(1,13):
            ...
            rmse = model_xgb.eval(matrix_test).split(':')[1]
            ...
            path_model = 'models'
            model_name = 'model_{0}_{1}_{2}_month.pkl'.format(model_type, target, str(i))
            model_file = os.path.join(path_model, model_name)
            #pickle.dump(model_xgb, open(path_model + model_name, 'wb'))
            os.makedirs(path_model, exist_ok=True)
            joblib.dump(value=model_xgb, filename=model_file)
        ...
        for i in range(1,13):
            ...
            model_list.append(model_file)
            rmse_list.append(rmse)
    return name_list, model_list, rmse_list
